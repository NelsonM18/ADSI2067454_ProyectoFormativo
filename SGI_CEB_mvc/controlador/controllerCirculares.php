<?php
include_once($_SERVER['DOCUMENT_ROOT']."/SGI_CEB_mvc/modelo/classCirculares.php");
...
if (isset($_REQUEST['insertarCircular'])) {
    if (is_uploaded_file($_FILES['circular']['tmp_name'])) { //Saber si el archivo si esta adjuntado
        ...
        if(!in_array($extension, $formatos_permitidos) ) {
            ...
            $ruta = $_SERVER['DOCUMENT_ROOT']."/SGI_CEB_mvc/vista/admin/circulares/"; //Direccion de la carpeta de las circulares
...
if (isset($_REQUEST['editarCircular'])) {
    ...
    if (is_uploaded_file($_FILES['circular']['tmp_name'])) { //Saber si el archivo si esta adjuntado
        ...
        if (is_file($upload)) { //Comprobar si ese archivo ya existe
            ...
            echo "<script>alert('Circular reemplazada correctamente');  window.location='../vista/admin/gestionarCirculares.php'; </script>";
            }
            else {
        ...
        echo "<script>alert('Circular reemplazada correctamente, no se cambio el archivo');  window.location='../vista/admin/gestionarCirculares.php'; </script>";
    }
...
if (isset($_GET['id_circularDELETE'])) {
    ...
    echo "<script>alert('Circular eliminada correctamente'); window.location='../admin/gestionarCirculares.php';</script>";
