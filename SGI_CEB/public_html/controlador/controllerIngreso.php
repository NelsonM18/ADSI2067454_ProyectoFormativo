...
include __DIR__."/../modelo/classHistorial.php";
...
if (isset($_REQUEST['insertarIngreso'])) {
    //Validar que el documento exista en la base de datos de las personas
    $num_documento= $_REQUEST['num_documento'];
    $res = $persona->validarPersona($num_documento);
    
    if ($res->num_rows==0) {
        echo "<script>alert('El documento no esta registrado');  window.location=document.referrer; </script>";
